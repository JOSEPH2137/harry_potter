...
public class Level5 {
    public static void level5(Player player) {
        printText("tu dois réussir à distraire Dolores Ombrage en attendant que tes amis prépare les feux d'artifices. Attention à ne pas mettre Ombrage trop en colère.");
        ...
        while (stillAlive == true) {
            ...
            if (player.pv < 1) {
                ...
                stillAlive = false;
                printText("DoloresOmbrage est tellement en colère qu'elle se déplace dans toute la salle et découvre tes amis en train de préparer les feux d'artifices.");
                defeat(player);
            }
    ...
    public static void paperplane(Player player,Boss doloresOmbrage, int paperplanetime){
        if (paperplanetime>2){
            printText("Dolores Ombrage a remarquée que tu lançais des avions en papier. Tu reçois une punition");
            ...
            printText("Dolores Ombrage ne comprend pas qui lance les avions. Cela l'énerve un peu.");
    ...
    public static void askquestion(Player player, Boss doloresOmbrage, int askquestiontime){
        if (askquestiontime>2){
            printText("Dolores Ombrage trouve que tu poses trop de questions. Tu reçois une punition");
            player.pv=player.pv-doloresOmbrage.attack/3;
            doloresOmbrage.attack=doloresOmbrage.attack + 10;
            doloresOmbrage.pv=doloresOmbrage.pv -5;
        }
        else{
            printText("Dolores Ombrage te réponds, mais cela l'agace beaucoup.");
            doloresOmbrage.attack=doloresOmbrage.attack + 25;
            doloresOmbrage.pv=doloresOmbrage.pv -10;
        }
    }
    public static void nothing(Player player){
        printText("La salle est tellement calme que Dolores Ombrage décide de sortir dans le couloir. Elle remarque tes amis en train de préparer les feux d'artifices");
        player.pv=0;
    }
    public static void spell(){
        printText("Tu n'as pas ta baguette magique dans la salle d'examen. Donc tu ne peux pas");
    }
    public static void complimente( Boss doloresOmbrage){
        printText("Tu fais un compliment à Dolores Ombrage. Cela lui fait plaisir et elle se calme un peu.");
        doloresOmbrage.attack=doloresOmbrage.attack-10;
        doloresOmbrage.pv=doloresOmbrage.pv -3;
    }
    public static void victory(Player player) {
        printText("Félicitations, tes amis on pu préparer les feux d'artifices. Que la fête commence !!!");
        makePotion(player,6);
    }
    public static void defeat(Player player) {
        printText("tu n'as pas réussi à distraire Dolores Ombrage. Elle a remarqué que vous prépariez une bêtise avec tes camarades. vous êtes tous puni.");
        gameOver(player,5);
